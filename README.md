...
merging to `main`. Production publishing runs only after a merged `release/*`
or `hotfix/*` pull request, or through a manual invocation with a full commit
SHA already present on `main`. Automatic deployments remain disabled unless
the `PRODUCTION_DEPLOYMENTS_ENABLED` repository variable is exactly `true`.
