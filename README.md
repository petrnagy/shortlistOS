...
The worker reads the same root `.env` file as the web app.

```bash
pnpm --filter @kan/shortlist-worker start
```

Available one-shot or dedicated jobs include:

```bash
pnpm --filter @kan/shortlist-worker process-sources
pnpm --filter @kan/shortlist-worker process-clip
pnpm --filter @kan/shortlist-worker process-inbox
pnpm --filter @kan/shortlist-worker prepare-enrichment
pnpm --filter @kan/shortlist-worker process-enrichment
pnpm --filter @kan/shortlist-worker start-enrichment
pnpm --filter @kan/shortlist-worker cleanup-openwebninja-cache
```
